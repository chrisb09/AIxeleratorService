...
#ifdef AIX_HAS_CUDA_RUNTIME
template<typename T>
struct TorchInference<T>::RangePipeline
{
    struct Request {
        uint64_t id;
        ...
        int first_rank;
        int end_rank;
        uint64_t timeline_step = 0;
        int timeline_world_rank = -1;
        int timeline_workgroup_rank = -1;
    ...
    RangePipeline(int device_id, int depth)
        : device(torch::kCUDA, device_id), max_active_requests(std::max(1, depth)),
          h2d_stream(c10::cuda::getStreamFromPool(false, device_id)),
          compute_stream(c10::cuda::getStreamFromPool(false, device_id)),
          d2h_stream(c10::cuda::getStreamFromPool(false, device_id))
    {
    }
    ...
    c10::cuda::CUDAStream h2d_stream;
    c10::cuda::CUDAStream compute_stream;
    c10::cuda::CUDAStream d2h_stream;
...
template<typename T>
TorchInference<T>::TorchInference() = default;
...
#ifdef AIX_HAS_CUDA_RUNTIME
    const char* pipeline_setting = std::getenv("AIX_P2P_RANGE_PIPELINE");
    const bool range_pipeline_enabled = device_id_ > -1 &&
        (pipeline_setting == nullptr || std::string(pipeline_setting) != "0");
    if (range_pipeline_enabled) {
        const int depth = std::max(1, aixelerator_service::utils::p2pTimelineEnvironmentInt(
            "AIX_P2P_RANGE_PIPELINE_DEPTH", 3));
        range_pipeline_ = std::make_unique<RangePipeline>(device_id_, depth);
        torch_model_.eval();
    }
#endif
}
...
template<typename T>
void TorchInference<T>::setControllerDirectBuffers(T* input_worker, T* output_worker)
{
    controller_input_worker_ = input_worker;
    controller_output_worker_ = output_worker;
}
...
template<typename T>
bool TorchInference<T>::supportsRangePipeline() const
{
#ifdef AIX_HAS_CUDA_RUNTIME
    return range_pipeline_ != nullptr;
#else
    return false;
#endif
}
...
template<typename T>
bool TorchInference<T>::canSubmitRangePipeline() const
{
#ifdef AIX_HAS_CUDA_RUNTIME
    if (!range_pipeline_) {
        return false;
    }
    if (static_cast<int>(range_pipeline_->requests.size()) >= range_pipeline_->max_active_requests) {
        return false;
    }
    if (!range_pipeline_->requests.empty()) {
        const auto& last_req = range_pipeline_->requests.back();
        if (!last_req.h2d_end.query()) {
            return false;
        }
    }
    return true;
...
template<typename T>
int64_t TorchInference<T>::maxRangePipelineSamples() const
{
    return batchsize_ > 0 ? batchsize_ : std::numeric_limits<int64_t>::max();
}

template<typename T>
uint64_t TorchInference<T>::submitRangePipeline(int64_t start_sample, int64_t sample_count,
                                                 int first_rank, int end_rank)
{
#ifndef AIX_HAS_CUDA_RUNTIME
    (void)start_sample;
    (void)sample_count;
    (void)first_rank;
    (void)end_rank;
    throw std::runtime_error("Range pipelining requires the CUDA runtime.");
#else
    if (!canSubmitRangePipeline()) {
        throw std::runtime_error("No free request slot in the CUDA range pipeline.");
    }
    if (start_sample < 0 || sample_count <= 0 || start_sample + sample_count > input_.size(0)) {
        throw std::out_of_range("Pipelined Torch range is outside the configured input tensor.");
    }

    auto& pipeline = *range_pipeline_;
    if (pipeline.requests.empty()) {
        last_timing_ = {};
    }
    auto& request = pipeline.requests.emplace_back();
    request.id = pipeline.next_request_id++;
    request.sample_start = start_sample;
    request.sample_count = sample_count;
    request.first_rank = first_rank;
    request.end_rank = end_rank;
    request.timeline_step = static_cast<uint64_t>(
        aixelerator_service::utils::p2pTimelineEnvironmentInt64("AIX_P2P_TIMELINE_STEP", 0));
    request.timeline_world_rank = aixelerator_service::utils::p2pTimelineEnvironmentInt(
        "AIX_P2P_TIMELINE_WORLD_RANK");
    request.timeline_workgroup_rank = aixelerator_service::utils::p2pTimelineEnvironmentInt(
        "AIX_P2P_TIMELINE_WORKGROUP_RANK");
    request.timeline_enabled = request.timeline_world_rank >= 0 &&
        std::getenv("AIX_P2P_TIMELINE_DIR") != nullptr;
    request.timing_enabled = request.timeline_enabled || std::getenv("AIX_DIAGNOSTICS") != nullptr;

    const auto emit = [&](double time_s, const char* event) {
        if (!request.timeline_enabled) {
            return;
        }
        aixelerator_service::utils::writeP2PTimelineEventAt(
            request.timeline_step, time_s, request.timeline_world_rank, request.timeline_workgroup_rank, true,
            event, -1, request.first_rank, request.end_rank, request.sample_start, request.sample_count);
    };

    const auto input_range = input_.narrow(0, start_sample, sample_count);
    if (first_rank == 0 && controller_input_worker_ != nullptr && controller_output_worker_ != nullptr) {
        std::vector<int64_t> direct_input_sizes = input_.sizes().vec();
        direct_input_sizes[0] = sample_count;
        request.input_cpu = torch::from_blob(
            static_cast<void*>(controller_input_worker_), direct_input_sizes, input_.options().device(torch::kCPU));
    }
    else if (input_is_pinned_) {
        request.input_cpu = input_range;
    }
    else {
        if (request.timeline_enabled) emit(aixelerator_service::utils::p2pTimelineTime(), "torch_input_stage_alloc_start");
        request.input_cpu = torch::empty_like(
            input_range, input_range.options().device(torch::kCPU).pinned_memory(true));
        if (request.timeline_enabled) emit(aixelerator_service::utils::p2pTimelineTime(), "torch_input_stage_alloc_end");
        if (request.timeline_enabled) emit(aixelerator_service::utils::p2pTimelineTime(), "torch_input_stage_copy_start");
        request.input_cpu.copy_(input_range);
        if (request.timeline_enabled) emit(aixelerator_service::utils::p2pTimelineTime(), "torch_input_stage_copy_end");
    }

    {
        c10::cuda::CUDAStreamGuard guard(pipeline.h2d_stream);
        if (request.timing_enabled) {
            request.timeline_anchor_s = aixelerator_service::utils::p2pTimelineTime();
            request.h2d_start.record(pipeline.h2d_stream);
        }
        request.input_gpu = request.input_cpu.to(
            request.input_cpu.options().device(pipeline.device).pinned_memory(false), true);
        if (request.timing_enabled) request.h2d_end.record(pipeline.h2d_stream);
        request.input_ready.record(pipeline.h2d_stream);
    }

    {
        c10::cuda::CUDAStreamGuard guard(pipeline.compute_stream);
        request.input_ready.block(pipeline.compute_stream);
        std::vector<torch::jit::IValue> inputs = {request.input_gpu};
        torch::NoGradGuard no_grad;
        if (request.timing_enabled) request.forward_start.record(pipeline.compute_stream);
        request.output_gpu = torch_model_.forward(inputs).toTensor();
        if (request.timing_enabled) request.forward_end.record(pipeline.compute_stream);
        request.compute_done.record(pipeline.compute_stream);
    }

    {
        c10::cuda::CUDAStreamGuard guard(pipeline.d2h_stream);
        request.compute_done.block(pipeline.d2h_stream);
        if (first_rank == 0 && controller_input_worker_ != nullptr && controller_output_worker_ != nullptr) {
            std::vector<int64_t> direct_output_sizes = output_.sizes().vec();
            direct_output_sizes[0] = sample_count;
            request.output_cpu = torch::from_blob(
                static_cast<void*>(controller_output_worker_), direct_output_sizes, output_.options().device(torch::kCPU));
            request.output_in_controller_buffer = true;
        }
        else {
            request.output_in_controller_buffer = output_is_pinned_;
            if (request.output_in_controller_buffer) {
                request.output_cpu = output_.narrow(0, start_sample, sample_count);
            }
            else {
                if (request.timeline_enabled) emit(aixelerator_service::utils::p2pTimelineTime(), "torch_output_stage_alloc_start");
                request.output_cpu = torch::empty_like(
                    request.output_gpu, request.output_gpu.options().device(torch::kCPU).pinned_memory(true));
                if (request.timeline_enabled) emit(aixelerator_service::utils::p2pTimelineTime(), "torch_output_stage_alloc_end");
            }
        }
        if (request.timing_enabled) request.d2h_start.record(pipeline.d2h_stream);
        request.output_cpu.copy_(request.output_gpu, true);
        if (request.timing_enabled) request.d2h_end.record(pipeline.d2h_stream);
        request.output_ready.record(pipeline.d2h_stream);
    }
...
template<typename T>
bool TorchInference<T>::rangePipelineComplete(uint64_t request_id)
{
#ifndef AIX_HAS_CUDA_RUNTIME
    (void)request_id;
    return false;
    ...
    auto& requests = range_pipeline_->requests;
    const auto iterator = std::find_if(requests.begin(), requests.end(),
                                       [request_id](const auto& request) { return request.id == request_id; });
    if (iterator == requests.end()) {
        throw std::out_of_range("Unknown CUDA range pipeline request.");
    }
    auto& request = *iterator;
    if (request.completed) {
        return true;
    }
    if (!request.output_ready.query()) {
        return false;
    }
    ...
    if (!request.output_in_controller_buffer) {
        if (request.timeline_enabled) {
            aixelerator_service::utils::writeP2PTimelineEvent(
                request.timeline_step, request.timeline_world_rank, request.timeline_workgroup_rank, true,
                "torch_controller_output_copy_start", -1, request.first_rank, request.end_rank,
                request.sample_start, request.sample_count);
        }
        output_.narrow(0, request.sample_start, request.sample_count).copy_(request.output_cpu);
        if (request.timeline_enabled) {
            aixelerator_service::utils::writeP2PTimelineEvent(
                request.timeline_step, request.timeline_world_rank, request.timeline_workgroup_rank, true,
                "torch_controller_output_copy_end", -1, request.first_rank, request.end_rank,
                request.sample_start, request.sample_count);
        }
    }

    if (request.timing_enabled) {
        const float h2d_ms = request.h2d_start.elapsed_time(request.h2d_end);
        const float forward_ms = request.forward_start.elapsed_time(request.forward_end);
        const float d2h_ms = request.d2h_start.elapsed_time(request.d2h_end);
        if (std::getenv("AIX_DIAGNOSTICS") != nullptr) {
            last_timing_.h2d_gpu_ms += h2d_ms;
            last_timing_.forward_gpu_ms += forward_ms;
            last_timing_.d2h_gpu_ms += d2h_ms;
            ++last_timing_.device_batches;
        ...
        if (request.timeline_enabled) {
            const double milliseconds_to_seconds = 1.0e-3;
            const auto phase_time = [&](const at::cuda::CUDAEvent& event) {
                return request.timeline_anchor_s + milliseconds_to_seconds * request.h2d_start.elapsed_time(event);
            };
            const auto emit = [&](double time_s, const char* event) {
                aixelerator_service::utils::writeP2PTimelineEventAt(
                    request.timeline_step, time_s, request.timeline_world_rank, request.timeline_workgroup_rank,
                    true, event, -1, request.first_rank, request.end_rank,
                    request.sample_start, request.sample_count);
            };
            const double h2d_start_s = request.timeline_anchor_s;
            const double forward_start_s = phase_time(request.forward_start);
            const double d2h_start_s = phase_time(request.d2h_start);
            emit(h2d_start_s, "torch_h2d_start");
            emit(h2d_start_s + milliseconds_to_seconds * h2d_ms, "torch_h2d_end");
            emit(forward_start_s, "torch_forward_start");
            emit(forward_start_s + milliseconds_to_seconds * forward_ms, "torch_forward_end");
            emit(d2h_start_s, "torch_d2h_start");
            emit(d2h_start_s + milliseconds_to_seconds * d2h_ms, "torch_d2h_end");
        }
    }
...
template<typename T>
void TorchInference<T>::releaseRangePipeline(uint64_t request_id)
{
#ifndef AIX_HAS_CUDA_RUNTIME
    (void)request_id;
#else
    ...
    const auto iterator = std::find_if(requests.begin(), requests.end(),
                                       [request_id](const auto& request) { return request.id == request_id; });
    if (iterator == requests.end() || !iterator->completed) {
        throw std::runtime_error("Cannot release an incomplete CUDA range pipeline request.");
    }
